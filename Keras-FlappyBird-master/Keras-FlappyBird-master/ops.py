...
def conv2D(x,shape,name,stride=[1,2,2,1],padding='SAME',reuse=None):
	...
	#compute convolution
	with tf.compat.v1.variable_scope('conv2d_'+name,reuse=reuse):
		kernel = tf.compat.v1.get_variable(name='kernel',
									shape=shape,
									initializer=tf.compat.v1.truncated_normal_initializer(mean=0.,stddev=.01))
		y = tf.nn.conv2d(x,kernel,stride,padding)
	return y
...
def dense(x,num_nodes,activation='relu',name='',bias_init_val=0,reuse=None):
    ...
    with tf.compat.v1.variable_scope('dense',reuse=reuse):
    	W=tf.compat.v1.get_variable('W'+name,initializer=tf.compat.v1.random_normal(stddev=.01,shape=[int(input_shape[-1]),num_nodes]))
    	b=tf.compat.v1.get_variable('b'+name,initializer=tf.compat.v1.constant(bias_init_val,shape = [num_nodes],dtype=tf.float32))
