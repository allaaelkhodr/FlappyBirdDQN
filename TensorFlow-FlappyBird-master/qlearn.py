...
def trainNetwork(model,args):
    ...
    image_input = tf.compat.v1.placeholder(dtype=tf.float32,shape=(None, 80, 80, 4)) #1*80*80*4
    Q_model = tf_buildmodel(image_input,reuse=None) #$$$

    global_step = tf.Variable(0.,trainable=False)
    tf_targets = tf.compat.v1.placeholder(dtype=tf.float32,shape=(BATCH,ACTIONS))
    tf_actions = tf.compat.v1.placeholder(dtype=tf.float32,shape=(BATCH,ACTIONS))
    tf_loss = tf.compat.v1.reduce_mean(tf.compat.v1.squared_difference(Q_model*tf_actions, tf_targets))
    tf_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=LEARNING_RATE).minimize(tf_loss,global_step=global_step)
